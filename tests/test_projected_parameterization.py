...
def test_projected_parameterization():
    ...
    for _i in tqdm(range(2_000)):
        ...
        # create a dummy planet to go through __init__ but avoid recompiling lightcurve
        _tmp_planet = OblateSystem(**injected_state)

        lc1 = planet1.lightcurve(params)

        # the 2d planet
        injected_state2 = {
            "projected_effective_r": _tmp_planet._state["projected_effective_r"][0],
            "projected_f": _tmp_planet._state["projected_f"],
            "projected_theta": _tmp_planet._state["projected_theta"],
        }
